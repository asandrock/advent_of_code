program main
  ...
  integer(I8P), allocatable :: seeds(:), locations(:)
  ...
  integer(I8P) :: j, old_empty(1), empty(1)
  ...
  allocate(locations(size(seeds)))
  do j = 1, size(seeds)
    locations(j) = mapping(humidity_to_location, &
      ...
      mapping(seed_to_soil, seeds(j))))))))
    !print *, seeds(j), location
  end do
  print *, minval(locations)
contains
